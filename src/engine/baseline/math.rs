...
///
/// `num_swa_layers` is a subset of the KV-bearing layers (`num_kv_layers.or(num_layers)`).
/// Windowed-dense models (Gemma 3): all layers KV-bearing, some windowed. Hybrid models: `num_kv_layers < num_layers` because linear layers carry no KV; a hybrid with windowed attention layers would count `num_swa_layers` against attention layers only.
/// `checked_sub` declines if a catalog entry ever violates the subset.
